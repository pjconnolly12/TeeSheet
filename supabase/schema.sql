...
create or replace function public.user_owns_round(target_round_id uuid)
returns boolean
language sql
security definer
set search_path = public
...
grant execute on function public.user_owns_round(uuid) to authenticated;
grant execute on function public.user_owns_round(uuid) to service_role;
...
drop policy if exists "Users can view their invitations" on public.round_invitations;
...
using (
  ...
  or public.user_owns_round(round_id)
);
